...
## CHANGE THIS   
#"one comment already, since I am looking at it right now. In the readme “~/MASCARA/R” , the repo (clone) most likely doesn’t live there. Maybe start from ‘clone this repo’ and then let people set the wd to where they put the repo. Or put the snippet from the readme into an r script that you let them open in rstudio to make sure they’re in the right place" AHB
```
...
<!-- 1. Initialise \( \mathbf{\tilde{Y}}_0 \) to \( \mathbf{\tilde{Y}} \) and \( \mathbf{B}_0 \) to \( \mathbf{B} \). -->
<!-- 2. Iteratively perform the following steps for \( k = 0, 1, 2, \ldots \): -->
...
<!-- and $\mathbf{v}_{k}^T\mathbf{v}_{k{}} = 1$ -->
<!-- PLS2 aims to maximize the covariance between the scores \( \bm{t}_k \) and \( \bm{w}_k \) at each iteration. -->
...
Rep_Tests <- readRDS("Noise_Sim_24_01_22_Replicate_Tests.RDS")  #_3  Noise_Sim_23_06_23_Replicate_Tests.RDS
