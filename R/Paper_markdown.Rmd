...
# author: Fred T.G. White^[1,2], Anna Heintz-Buschart^[1], Lemeng Dong^[2], Harro J. Bouwmeester^[2], Johan A. Westerhuis^[1], Age K. Smilde^[1]

# author: "Fred T.G. White, Anna Heintz-Buschart, Lemeng Dong, Harro J. Bouwmeester, Johan A. Westerhuis, Age K. Smilde"


# Fred T.G. White^[Biosystems Data Analysis Group, Swammerdam Institute for Life Sciences, University of Amsterdam, Amsterdam, The Netherlands, Plant Hormone Biology Group, Swammerdam Institute for Life Sciences, University of Amsterdam, Amsterdam, The Netherlands *], Anna Heintz-Buschart^[Biosystems Data Analysis Group, Swammerdam Institute for Life Sciences, University of Amsterdam, Amsterdam, The Netherlands], Lemeng Dong^[Plant Hormone Biology Group, Swammerdam Institute for Life Sciences, University of Amsterdam, Amsterdam, The Netherlands], Harro J. Bouwmeester^[Plant Hormone Biology Group, Swammerdam Institute for Life Sciences, University of Amsterdam, Amsterdam, The Netherlands], Johan A. Westerhuis^[Biosystems Data Analysis Group, Swammerdam Institute for Life Sciences, University of Amsterdam, Amsterdam, The Netherlands], Age K. Smilde^[Biosystems Data Analysis Group, Swammerdam Institute for Life Sciences, University of Amsterdam, Amsterdam, The Netherlands]


# - name: Fred T.G. White
#     email: f.t.g.white@uva.nl
#     affiliation: 
...
<!-- For the rest of the manuscript we take the strigolactones as our example biological pathway. -->
...
Pearson's correlation coefficient *r* is calculated between all columns of \(\bm{Y}\). Ranks are assigned to all non-bait transcripts for each bait based on *r*, with the lowest rank for the highest correlation. These ranks are averaged across all baits (see supplementary equation \@ref(eq:rankcor)).
...
\begin{equation}
\begin{aligned}
\mathbf{M}_f=\left[0|\mathbf{X}_f |0\right]\mathbf{\hat{\Theta}} = \mathbf{X}_f \mathbf{\hat{\Theta}}_f
\end{aligned}(\#eq:scim)
\end{equation}
...
\mathbf{\hat{Y}} =& \mathbf{M}_{\alpha} + \mathbf{M}_{\alpha\beta}
                =& \mathbf{T}_{\alpha}\mathbf{P}_{\alpha}^T + \mathbf{T}_{\alpha\beta}\mathbf{P}_{\alpha\beta}^T
...
2. Iteratively perform the following steps for \( k = 1, 2, \ldots \):
   a. Compute the singular value decomposition (SVD) of \( \mathbf{\tilde{Y}}_{k}^{T} \mathbf{B}_{k} \) to obtain the left and right singular vectors; weights \( \mathbf{u}_k \) and \( \mathbf{v}_k \) respectively.
...
Under the conditions of orthogonal scores; $\mathbf{t}_{k}^{T}\mathbf{t}_{k} = 0$ and normalised weights; $\mathbf{u}_{k}^T\mathbf{u}_{k{}} = 1$ and $\mathbf{v}_{k}^T\mathbf{v}_{k{}} = 1$.
...
To evaluate performance we use log2 geometric mean rank (log2(GMR); $\bar{\gamma}$), defined as: 
...
for $J_{spike}$ spikes. A lower $\gamma$ indicates better performance.
...
for(i in orders){
  ...
  One_Balance[[i]] <- ggplot(mRES[which(mRES$nCERs == orders[i]),], aes(x = Replicate_Number, y = log2(F1_Score), colour = Method, shape = Method)) +
...
```{r repres, echo = F, fig.cap= "Simulation 2 results. Replications and variance type feasibility ranges. X axes; replicate number, y axes log2 transformed geometric mean rank. Each facet is created with set parameters for combined effect size ($CE = \\|\\mathbf{T}_{\\alpha + \\alpha\\beta}\\mathbf{P}_{\\alpha + \\alpha\\beta}^T\\|_F$) and random gaussian noise size ($E = \\|\\mathbf{E}\\|_F$). Dots are median performance across 20 random structures.  ASCA model calculated per equation \\@ref(eq:ascamodel), PLS model per equation \\@ref(eq:plsmodel)."}
...
We acknowledge funding by the Dutch Research Council (NWO/OCW) for the MiCRop Consortium program, Harnessing the second genome of plants (Grant number 024.004.014; to HB, LD, AKS, JAW), the Dutch Research Council (NWO-TTW grant 16873 Holland Innovative Potato; to HB and LD) and the Data Science Centre of the University of Amsterdam (to FW). FW would like to thank Frans van der Kloet, Fentaw Abegaz, Roel van der Ploeg for statistical discussions and Imran Haider for consultation on the strigolactone pathway.
...
\begin{equation}
\begin{aligned}
Rank_{\tilde{J}} = \frac{\sum_{b=1}^{B}rank_{b\tilde{j}}}{B}  
