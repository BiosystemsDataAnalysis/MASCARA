...
```{r nettype, fig.cap= c("Pathway types exemplified, POI; pathway of interest, RP; related pathway, URP; unrelated pathway. Panel A represents hypothetical biological pathways  that are all affected by the experimental design and thus all contain DE genes, here the edges represent real connections between the nodes/genes. Panel B highlights the problem with detecting these pathways when dealing with data from designed experiments, here edges represent putative connections defined by coexpression analysis; black is desired (within network detection), grey is non-desired.")}

a <- knitr::include_graphics("Fig1.tif")
a
```
...
heat2 <- knitr::include_graphics("Fig3.tif")
heat2
```



```{r ASCA-1, echo = F, out.width = '100%', fig.cap=c("Scores and loadings of the ASCA model of the combined effect (condition + condition:time interaction). Scores: *g_c* indicates growth condition, *time* indicates days after starvation induction. Loadings: POI; SL genes and URP; other highly differential genes (indicated in purple and green respectively) show upregulation in the P- condition compared  to P+.")}
knitr::include_graphics("Fig4.tif")
```
...
The simulations are built upon the results of the ASCA analysis of the combined effect of growth condition (g_c) and the interaction of growth condition and time. As can be seen in Figure \@ref(fig:ASCA-1) there is clear separation between samples due to the experimental factors growth condition and time. The variation between the replicates is only small. The green loadings of URP genes are strongly related to the experimental variation due to g_c. The simulated data is created by simulating experimental variation consisting of the main time effect $\beta$ and the combined effect ($\alpha + \alpha\beta$), and the Ambient/Residual variation which exists of the structured ambient variance ($\mathbf{S}$) and residual variation ($\mathbf{E}$) representing measurement error, modeled with normally distributed random values $e_{ij} = \mathcal{N}(0,\sigma^2)$.
...
In the loading matrices $\mathbf{P}$, the different coloured blocks indicate different types of genes. The green block represents the bait genes of POI. The black genes represent “unknown” target genes from POI or RP. Yellow and purple represent genes from URP that are affected by the experimental variation in different ways. The grey blocks represent very small background variation for those genes, while the white blocks represent that those genes are not affected by that source of background variation. 

In $\mathbf{P}_{\beta 1}^T$ the baits, targets and other DE genes (all genes from POI, RP and URP) along with the remaining first half have similar positive values (represented by light grey) where the other 1000 have positive values in $\mathbf{P}_{\beta 2}^T$ (grey), these are created to emulate the reality of the real data in which many genes are slightly correlated with eachother through the course of development, as well as to account for some nonlinearities which are otherwise ignored by our model. The remaining elements of $\mathbf{P}_{\alpha + \alpha \beta}^T$ and $\mathbf{P}_{\beta}^T$ are populated with zeroes. $\mathbf{P}_S$ only has values for the baits (green) and target (black) genes. The loading values for the baits and targets are obtained as $p_{S_jk}^{baits,targets} \sim \mathcal{N}(0.5,0.2)$, while the loadings for the other genes are 0. 
...
```{r simOverview, echo=FALSE, fig.cap="Coexpression simulation, simplified overview. Colours correspond to the same genes in the different loading vectors ($\\mathbf{P}$). Black; baits (known POI genes), green; targets (unknown POI genes), yellow; DE URP genes, purple DE URP genes with a different profile. In the score vectors ($\\mathbf{T}$); Blues, reds, greys and golds represent experimental conditions, oranges and pinks; time. Each of the textures in the ambient/residual part represent a distinct normally distributed set of random values.", out.width = '100%'}
knitr::include_graphics("Fig5.tif")
```
...
In this simulation we have a set of 4 baits, 12 target genes and a variable number URP genes. The issue to address is the presence of many differential URPs; those that have strong between group correlation but no within group correlation with the POI. We incrementally increase the number of genes in the yellow block and decrease number of white genes with 0 loadings. As well as the number of DE URPs we also independently control the size of the experimental variance through the parameter *d* in Equation in \@ref(eq:nCER). This approach controls the size of the experimental variance as well as the number of genes responding while keeping a fixed amount of ambient or within group variation.
...
```{r, echo = F}
#create "ref" for feature target information
...
1. For the $1^{st}$ component:
   a. Compute the left singular vector $\mathbf{u}_1$ of $\mathbf{\tilde{Y}}^{T} \mathbf{B}$
   b. $\mathbf{t}_1 = \mathbf{\tilde{Y}}\mathbf{u}; \mathbf{T} = \left [\mathbf{t}_1\right]$
   c. $\mathbf{p}_1 = \frac{\mathbf{\tilde{Y}}^T\mathbf{t}^t}{\mathbf{t}^t\mathbf{t}}; \mathbf{P} = \left [\mathbf{p}_1\right]$
2. For the following components $k = 2...K$:
   a. Compute the left singular vector $\mathbf{u}_k$ of $(\mathbf{I} - \mathbf{P}(\mathbf{P}^T\mathbf{P})^{-1}\mathbf{P})\mathbf{\tilde{Y}}^{T} \mathbf{B}$
...
Under the conditions of orthogonal scores; $\mathbf{t}_{k}^{T}\mathbf{t}_{k+1} = 0$ and normalised weights; $\mathbf{u}_{k}^T\mathbf{u}_{k{}} = 1$ . Loadings for $\mathbf{B}$ can be obtained with a least squares step $\mathbf{Q} = (\mathbf{T}^T\mathbf{T})^{-1}\mathbf{T}^T\mathbf{B}$.
...
\begin{equation}
\begin{aligned}
\bar{\gamma} = log_{2}((\prod_{j_{target}=1}^{J_{target}}\gamma_{j_{target}})^{1/J_{target}})
\end{aligned}(\#eq:GMR)
\end{equation}

for $J_{target}$ targets. A lower $\bar{\gamma}$ indicates better performance.
...
Noise_Tests <- readRDS("Noise_Sim_Coexp_24_06_13.RDS")    #Noise_Sim_Coexp_24_05_24.RDS

names(Noise_Tests) <- c("ASCA", "PLS", "Correlations", "MASCARA", "WGCNA")

Noise_Tests <- Noise_Tests[c("ASCA", "PLS", "Correlations", "WGCNA", "MASCARA")]
...
mRES$Noise <- rep(ERs, 5)
mRES$Main <- rep(main_lev, 5)
```
...
For this simulation we have some experimental variance for the combined effect and time ($\bm{T}_{\alpha + \alpha\beta}\bm{P}_{\alpha + \alpha\beta}^T$ and $\bm{T}_{\beta}\bm{P}_{\beta}^T$ respectively) a fixed amount of random noise \(\bm{E}\) and a fixed amount of structured ambient/residual variance $\bm{T}_S\bm{P}_{S}^T$, within which our 4 baits and 12 targets share a correlation structure. This reflects the situation expected from the real data example - drastic effect caused by nutrient deficiency, which also activates the SL pathway. The POI genes are expected to share some variance that is independent to the experiment, the 4 baits and 12 targets load on to the structured part of the noise $\bm{T}_S\bm{P}_{S}^T$.
...
# tiff("Fig6.tif", res = 150, height = 960, width = 960)
# wrap_plots(plots) + plot_layout(guides = "collect")
# dev.off()
```
...
Rep_Tests <- readRDS("Noise_Sim_24_12_08_Replicate_Tests_3.RDS") #Noise_Sim_24_12_04_Replicate_Tests.RDS  #Noise_Sim_24_06_13_Replicate_Tests.RDS Noise_Sim_24_05_24_Replicate_Tests.RDS

names(Rep_Tests) <- c("ASCA", "PLS","Correlations","MASCARA", "WGCNA")
...
# orders <- c(7,8,9,4,5,6,1,2,3)      # - WARNING -
orders <- c(21,22,23,24,25,
            16,17,18,19,20,
            11,12,13,14,15,
            6,7,8,9,10,
            1,2,3,4,5)
...
# tiff("Fig7_new1.tif",width = 960*4, height = 960*4,pointsize = 12, res = 300)
# ps1 <- grid.arrange(ps, left = "log2(GMR)", bottom = "Replicate Number                     ")
# dev.off()

a <- knitr::include_graphics("Fig7_new1.tif")
a
...
It was expected that all geometric mean ranks converge towards a minimum with increasing numbers of replicates, see \@ref(fig:repres). Perfect detection of 12 targets in top 12 candidates is achieved with a log2 geometric mean rank of 2.4, MASCARA was able to achieve this, however, the other methods were perturbed by the presence of the 30 differential URP genes. With more replicates MASCARA always outperforms competitors. With higher values of *d* (larger combined effect size) (CE) MASCARA outperforms the other methods but its performance is affected by amount of random noise (E) size indicated by *l*. 

In simulation 2 we fixed the number of differential URP genes to 30, however this is less than 2% of the total number of genes in each dataset. In the real data example more than 10% of of the total number of genes are differentially expressed. As shown in simulation 1 (Figure \@ref(fig:tc)) higher numbers of DE URP genes results in higher log2 geometric mean rank (log2(GMR)) of the targets for correlations, ASCA and PLS.  MASCARA is designed specifically to mitigate the effect of large numbers of DE URP genes. Here we show that this is achieved.
...
```{r}
      # F_norms <- c(do.call(c,lapply(res_ASCAplus_nb$decomposition, function(x) norm(x[, c(pathway, non_pathway)], type = "F"))), 
      #            residual = norm(res_ASCAplus_nb$residuals[, c(pathway, non_pathway)], type = "F"))
      # 
...
m_res_full <- MASCARA(res_ASCAplus_nb$residuals,baits)
m_rest <- m_res_full[[3]]$scores %*% t(m_res_full[[3]]$loadings)
m_baits <- m_res_full[[3]]$Yscores %*% t(m_res_full[[3]]$Yloadings)

m_comb <- cbind.data.frame(m_rest, m_baits)

deflated_perc <- sum(m_res_full[[3]]$Xvar)/m_res_full[[3]]$Xtotvar
...
F_norms <- c(F_norms, 
             "CE" = norm(res_ASCAplus_nb$decomposition$growth_condition +
                           res_ASCAplus_nb$decomposition$`growth_condition:time`, type = "F"),
             "modelled" = deflated_perc * F_norms["residual"],
             "noise" = (1-deflated_perc) * F_norms["residual"] )   

F_norms[5:7] / sum(F_norms[5:7])
...
@tajiImportantRolesDrought2002 

@zintaDynamicsMetabolicResponses2018
...
GAs <- GAs[which(GAs %in% colnames(res_ASCAplus_nb$residuals))]
...
# Define the function to calculate Fisher-transformed correlation values
cor_val <- function(x){
  cor_res <- cor(x[])
  cor_res <- 0.5 * log((1 + cor_res) / (1 - cor_res))
  return(cor_res)
...
# Calculate correlations for res_ASCAplus_nb$residuals
# test_residuals <- cor_val(res_ASCAplus_nb$residuals[, pathway])

# pathway <- baits

ASCA_cands2_filtered <- ASCA_cands2[!rownames(ASCA_cands2) %in% pathway, ]

# non_pathway <- PSI_ref[,1]
# non_pathway <- non_pathway[which(non_pathway %in% rownames(ASCA_cands2_filtered))]

non_pathway <- rownames(ASCA_cands2_filtered[which(ASCA_cands2_filtered$VIP > quantile(ASCA_cands2_filtered$VIP, 0.9)),])#[1:1253]

test2_residuals <- cor_val(res_ASCAplus_nb$residuals[, c(pathway, non_pathway)])
test2_residuals[upper.tri(test2_residuals, diag = TRUE)] <- NA

mt_residuals <- melt(test2_residuals, na.rm = TRUE)

mt_residuals$group <- "Other"
mt_residuals$group[mt_residuals$Var1 %in% pathway & mt_residuals$Var2 %in% pathway] <- "SL-SL"
mt_residuals$group[mt_residuals$Var1 %in% non_pathway & mt_residuals$Var2 %in% pathway] <- "SL-Other"

mt_residuals <- mt_residuals[mt_residuals$group %in% c("SL-SL", "SL-Other"), ]
colnames(mt_residuals)[3] <- "Correlation"

mt_residuals$Dataset <- "Within"


# test_X <- cor_val(X[, pathway])

test2_X <- cor_val(X[, c(pathway, non_pathway)])
test2_X[upper.tri(test2_X, diag = TRUE)] <- NA

mt_X <- melt(test2_X, na.rm = TRUE)

mt_X$group <- "Other"
mt_X$group[mt_X$Var1 %in% pathway & mt_X$Var2 %in% pathway] <- "SL-SL"
mt_X$group[mt_X$Var1 %in% non_pathway & mt_X$Var2 %in% pathway] <- "SL-Other"

mt_X <- mt_X[mt_X$group %in% c("SL-SL", "SL-Other"), ]
colnames(mt_X)[3] <- "Correlation"
...
#   #target projection
#   spls_res <- simpls.fit(resids[,-which(colnames(resids) %in% baits)],
#                           resids[,which(colnames(resids) %in% baits)], ncomp = 3)
#   
#   #####
#   y <- colMeans(spls_res$Yloadings)
#   U <- spls_res$projection
...
# m_res <- MASCARA(res_ASCAplus_nb$residuals[, c(pathway, non_pathway)],baits)[[3]]
# 
# m_rest <- m_res$scores %*% t(m_res$loadings)
# m_baits <- m_res$Yscores %*% t(m_res$Yloadings)
# 
# m_comb <- cbind.data.frame(m_rest, m_baits)
# 
# test_m <- cor_val(m_comb[, c(pathway, non_pathway)])
# test_m[upper.tri(test_m, diag = TRUE)] <- NA
# 
# mt_m <- melt(test_m, na.rm = TRUE)
# 
# mt_m$group <- "Other"
# mt_m$group[mt_m$Var1 %in% pathway & mt_m$Var2 %in% pathway] <- "SL-SL"
# mt_m$group[mt_m$Var1 %in% non_pathway & mt_m$Var2 %in% pathway] <- "SL-Other"
# 
# mt_m <- mt_m[mt_m$group %in% c("SL-SL", "SL-Other"), ]
# colnames(mt_m)[3] <- "Correlation"
# 
# mt_m$Dataset <- "Deflated"
# 
...
mt_combined <- rbind(mt_residuals, mt_X)   #   , mt_m

mt_combined$Group_Dataset <- paste(mt_combined$group, mt_combined$Dataset, sep = "_")

mt_combined$Dataset <- factor(mt_combined$Dataset, levels = c("Total", "Within", "Deflated"))

dens1 <- ggplot(mt_combined, aes(x = group, y = Correlation, fill = group)) +
  geom_violin() +
  theme_bw() +
  labs(fill = "Group") +
  ggtitle("Boxplot of Fisher-Transformed Correlations by Variance Partition") +
  xlab("Group") +
  ylab("Correlation") +
  facet_wrap(~ Dataset) +
  stat_compare_means() +
  theme(
    ...
    legend.position = "none"  # Remove legend if 'fill' is the same as 'x'
  )
...
dens <- ggplot(mt_combined, aes(Correlation, fill = group)) + geom_histogram(bins = 140) + theme_bw()  + facet_wrap(~ Dataset)
...
# 
# xkl <- KL.divergence(mt_combined$Correlation[which(mt_combined$Group_Dataset == "SL-Other_Total")], mt_combined$Correlation[which(mt_combined$Group_Dataset == "SL-SL_Total")])
# 
# rkl <- KL.divergence(mt_combined$Correlation[which(mt_combined$Group_Dataset == "SL-Other_Within")], mt_combined$Correlation[which(mt_combined$Group_Dataset == "SL-SL_Within")])
# 
# plot(xkl, rkl)
# abline(a=0,b=1)
```
...
a <- knitr::include_graphics("Fig9.tif")
a
