...
<!-- Need to change this as it is repeated in introduction -->
...
Figure \@ref(fig:nettype) outlines pathway nomenclature that will be used throughout this text. *A* represents a true hypothetical pathway including a pathway of interest (POI), related pathway (RP) and an unrelated pathway (URP). All three types of pathway are assumed to be affected to varying degrees by the experimental design. When dealing with experimentally designed data, we have to consider two key sources of variance: between-group variance and within-group variance. Between-group variance pertains to the differences between experimental groups (or conditions). This type of variance is of interest when conducting DE studies. Within-group variance, on the other hand, is the variance within experimental groups, this is not interesting for DE studies but may contain information useful to CoE. It is the excessive between-group variance that leads to the densely connected nature of *B* from \@ref(fig:nettype). Figure \@ref(fig:withinbetween) exemplifies between-group and within-group driven correlations. The plots show the relationship between two genes of the POI (*A* and *C*) and the relationship between two genes that are not part of the same pathway (*B* and *D*).  The total correlations of both pairs of transcripts are high but the second pair’s correlation is driven by the between-group variation and not by correlations within experimental groups. The within-group correlation is lower than the total in both cases, but there is a clear trend in one condition (facet *C*: P- condition in red) for the first gene pair. Because of this, the within group correlation (r) is higher for the first transcript pair (*C*) than the second (*D*), which makes it possible to discern between genes within a POI or RP and genes that both respond similarly to a treatment (URP). Hence, we need to distinguish *total CoE* which is calculated on the total variance and *within-group CoE* where the focus is on the shared variance within experimental groups. 

<!-- From a plant biology point of view, the between-group variance and the within-group variance can be quite different and related to different parts of the biological system. It is, in fact, expected that there are mechanistic changes that arise from the very different conditions that could alter the coexpression of genes.  -->


<!-- , examples include: @miettinenSecoiridoidPathwayCatharanthus2014;.  -->
...
<!-- This phenomenon is well documented in @caldanaHighdensityKineticAnalysis2011 where an experiment was performed investigating the effects of temperature and light on arabidopsis plants over time. In this study the authors investigated variable relationships between metabolites and showed these were strongly dependent on the specific levels of the experimental factors, e.g., shikimic acid and phenylalanine were positively correlated at 21 degrees in high light condition, but negatively correlated at 32 degrees in the dark.  -->

<!-- Between experimental groups different pathways are active, whereas within groups more general (undocumented/uncontrolled) ambient factors such as temperature, light intensity and soil water content can affect the plant and its transcriptome in a concerted fashion. In experiments where the intervention causes large between-group differences to many genes, not making the distinction between types of variance can lead to results that contain total CoE associations driven by similar differences between treatment groups.  -->
...
```{r Dataset1-Overview, echo = F, fig.cap=c("Real data overview. RNAseq of rice root. Heatmap of SL pathway and example highly differential genes.")}
...
```{r ASCA-1, echo = F, out.width = '100%', fig.cap=c("Scores and loadings of the ASCA model of the combined effect (condition + condition:time interaction).")}
...
The first simulation aims to determine how well the methods perform to find coexpressed genes in the presence of a dominant between-group variance structure and increasing numbers of non-pathway differential genes (genes with high between-group correlation but no within-group correlation). The expression levels of differential transcripts is modeled with the scores and loadings of the growth condition and its interaction with time ( $\mathbf{T}_{\alpha + \alpha\beta}$ and $\mathbf{P}_{\alpha + \alpha\beta}^T$ ). The disturbing time effect is modeled with scores and loadings ( $\mathbf{T}_{\beta}$  and $\mathbf{P}_{\beta}^T$ ). Also included is some random measurement noise, this is modeled as a matrix $\mathbf{E}$ with normally distributed random values $e_{ij} = \mathcal{N}(0,\sigma^2)$. 

On top of the between-group effect there is an additional a correlation structure applied to the baits and spikes to create a model of coexpression that is independent to the variance induced by the experimental factors ($\mathbf{X}_S = \mathbf{T}_S\mathbf{P}_S^T$). In this simulation we have a set of 4 baits that have positive loadings for the combined effect along with a variable number of other genes - "Combined Effect Responders". For the structured ambient variance; 2 random sample offset vectors in \(\bm{T}_S\) (each column representing an effect caused by ambient conditions) which our baits and spikes contribute to (i.e. green and black bars in Figure \@ref(fig:simOverview) are non zero in \(\bm{P}_S\)). Similarly to the real data, the gene expression values for each experimental condition are simulated with three independent replicates per time-condition-combination. Each simulated data $\mathbf{Y}$ set thus consists of 24 samples and 2000 genes.
...
\text{where for each element ik or jk:}\\
...
<!-- ### Combined effect size and number of differential genes -->
...
<!-- \text{where:} &\\ -->
<!-- \bm{P}_{\alpha + \alpha\beta}^T = \begin{bmatrix} -->
<!--        \bm{p}_{11} & \bm{p}_{12} \quad \bm{p}_{13} \\ -->
<!--        \bm{p}_{21} & \bm{p}_{22} \quad \bm{p}_{23} -->
<!--      \end{bmatrix} -->
<!-- \end{aligned}(\#eq:nCER) -->
<!-- \end{equation} -->


The issue to address is the presence of many non-pathway differential genes; those that have strong between group correlations but no within group correlation. Here the differential genes relate  to the yellow bar in \@ref(fig:simOverview). We incrementally increase the number of non-zero values and decrease the number of 0 values, which in figure \@ref(fig:simOverview) relates to increasing the size of the yellow bar, as well as varying the F-norm of the combined effect matrix through the scalar parameter *d* below;
...
```{r, echo = FALSE}
# X_funced_classic <- Create_Core(nreps = 3, meta = meta, irr_spikes = TRUE,
#                         struc_resid = FALSE, noise_sd = 0.75, plot = TRUE)   ## sum(X_funced[[1]] != X_core) = 0 with these parameters
...
# nreps <- 5
# 
# meta2 <- cbind.data.frame(rep(c(1,-1), each = betaN*nreps), rep(c(1:betaN), each = nreps), rep(c(-1:-betaN,1:betaN), each = nreps))
# 
# colnames(meta2) <- c("growth_condition","time","interaction")
...
Pearson's correlation coefficient is calculated between all columns of \(\bm{Y}\). 
...
For ranked correlations *r* is calculated between each bait and all other genes, with the lowest rank for the highest correlation. Then for each bait the other genes are ranked based on *r*. These ranks are averaged across all baits, similarly to @poretskyMutRankShinyWebapplication2020.
...
<!-- This approach provides a general solution to build the ASCA effect matrices for any fixed effects ANOVA model. -->




### PLS regression - canonical mode


Partial Least Squares canonical mode (PLS2) is a multivariate regression technique used for modeling relationships between two sets of variables, \( \mathbf{Y} \) and \( \mathbf{B} \). 


After splitting the data into the response matrix (containing only baits) \( \mathbf{B} \) (the green bar in the loadings) and the predictor matrix (containing the rest of the data) \( \mathbf{Y} \), resulting in the model of the data:

\begin{equation}
\begin{aligned}
\mathbf{Y} = \mathbf{T}\mathbf{P}^T + \mathbf{E}_Y \\
\text{and} \\
...
This model is estimated in our work as follows:

<!-- 2. Center and scale both \( \mathbf{X} \) and \( \mathbf{Y} \). -->
1. Initialize \( \mathbf{Y}_0 \) to \( \mathbf{Y} \) and \( \mathbf{B}_0 \) to \( \mathbf{B} \).
2. Iteratively perform the following steps for \( k = 1, 2, \ldots \):
   a. Compute the singular value decomposition (SVD) of \( \mathbf{Y}_{k}^{T} \mathbf{B}_{k} \) to obtain the left and right singular vectors \( \mathbf{u}_k \) and \( \mathbf{v}_k \) respectively.
   b. Project \( \mathbf{Y}_{k} \) and \( \mathbf{B}_{k} \) onto the first left and right singular vectors (\( \mathbf{u}_k \), \( \mathbf{v}_k \)) to obtain the scores \( \bm{t}_k = \mathbf{Y}_{k} \mathbf{u}_k \) and \( \bm{w}_k = \mathbf{B}_{k} \mathbf{v}_k \).
   c. Perform linear regression of \( \mathbf{Y}_k \) and \( \mathbf{B}_k \) onto their corresponding scores \( \bm{t}_k \) and \( \bm{w}_k \) to obtain the respective loadings \( \bm{p}_k \), and \( \bm{q}_k \).
   d. Deflate \( \mathbf{Y}_k \) and \( \mathbf{B}_k \) by removing the variance explained by the $k^{th}$ pair of scores (\( \bm{t}_k \), \( \bm{w}_k \)), and use the deflated matrices as inputs for the next iteration.

PLS2 aims to maximize the covariance between the scores \( \bm{t}_k \) and \( \bm{w}_k \) at each iteration. In the following simulations and application the number of components calculated by PLS2 is set to 2. 

<!-- although there are methods to optimally select this number, such as; (REFS). -->
...
To evaluate performance we use log2 of the geometric mean rank ($\bar{\gamma}$), defined as: 
$\bar{\gamma} = log2((\prod_{i=1}^{n}\gamma_{i})^{1/n})$ 
for each spike *i* for n spikes.

For the real data, since the ground truth is not known we visualise relationships between baits and genes detected by the different methods rather than using a metric. 
...
```{r, echo=FALSE, include=FALSE}
db <- cbind.data.frame("Sample" = c(1:24),meta[,1:2],X)
...
# (dist_norm + plot_normal)  # no noise data so cannot plot mean sd relationships here
```
...
```{r, echo = FALSE}
# comb_terms <- ASCA_combine_terms(res_ASCAplus_nb, c("growth_condition","growth_condition:time"))
# svd_test <- ASCA_svd(comb_terms$decomposition$`growth_condition+growth_condition:time`)
#
...
```{r,echo = F}
# N_T <- readRDS("Noise_Sim_220909.RDS")

Noise_Tests <- readRDS("Noise_Sim_Coexp_23_07_26_11.RDS")  #Noise_Sim_Coexp_23_06_12.RDS -> updated Beta and VIP for PLS   ##Noise_Sim_Coexp_23_06_14_2.RDS
...
# y <- rep((c(0:20)/33.3333333333333333333333333333333))
# x <-  c(0:10) * 10
# 
# 
...
# Noise_Tests <- lapply(Noise_Tests, function(x) names(x) = names)
...
#####
# 
# RES$Noise <- gsub("_.*", "",RES$Cond)
# RES$Noise <- gsub("X","",RES$Noise)
# # RES$Resid <- gsub("E_","",RES$Resid)
# 
# # RES$Feature <- gsub(".*_","",RES$Cond)
# 
# RES$Main <- gsub(".*_","", RES$Cond)
# # RES$Main <- gsub("X","", RES$Main)
# 
mRES <- melt(RES)
...
```{r, echo=FALSE}
# mRES$Noise  <- as.numeric(mRES$Noise)
# mRES$Main  <- as.numeric(mRES$Main)
mRES$F1 <- as.numeric(mRES$F1)

colnames(mRES)[3] <- "Rank_Product"
mRES$Rank_Product <- log2(mRES$Rank_Product)
# colnames(mRES)[5] <- "Rank_Product"
...
max <- max(mRES$Rank_Product)
min <- min(mRES$Rank_Product)
...
for(i in 1:length(methods))
  plots[[i]] <- ggplot(mRES[which(mRES$Method == methods[i]),], aes(x = Noise, y = Size_num))+ #, colour = Rank_Product
  # geom_point( alpha = 1, size = 10, shape = 15) + #position = "jitter",
    geom_tile( aes(fill = Rank_Product)) + #position = "jitter",

  # scale_x_discrete(guide = guide_axis(angle = 90, check.overlap = T))+
  # scale_color_continuous(type = "viridis", limits = c(min,max))+
...
```{r tc, echo = F, fig.cap= "Results of simulation 1 for all 4 methods. The geometric mean rank as a function of the number of differentially expressed genes (x-axis) and combined effect size (y axis)."}
...
<!-- The results of simulation 1 were expected to show no effect for MASCARA,  due to the capturing and removal of the dominant experimental variance, but a decrease in performance for the other methods with increasing numbers of non-pathway DE genes and increasing combined effect size it becomes more difficult to detect the true coexpression pattern, as this is masked. -->

We can see in Figure \@ref(fig:tc) that MASCARA was neither affected by the presence of an increasing number of non-pathway DE genes nor the size of this effect. Correlations, ASCA and PLS, however, all showed the expected breakdown in performance due to the injection of more non-pathway DE genes. This was the expected result as the between group variance is removed with MASCARA.
...
# Dm <- mean(data.matrix(RF_RES[12,]))
# Em <- mean(data.matrix(COEXP_HIGH_RES[8,]))

# get F1 scores for 12 and 8 respectively for linegraph with error
...
# colnames(mRES)[2:3] <- c("Method","F1_Score")
# mRES$F1_Score[which(mRES$F1_Score == 0)] <- 0.001
...
# MR <- mRES
# 
# mRES <- mRES[-which(mRES$Method == "ASCA"),]
...
for(i in orders){
  ...
  # geom_line() +
    ...
    # geom_errorbar(mappings = aes(x = Replicate_Number, y = log2(F1_Score)),stat = "summary",
    #               fun.min = function(z) {quantile(z,0.25)},
    #               fun.max = function(z) {quantile(z,0.75)},
    #               fun = median) +
  # geom_smooth( se = F)+    # aes(outfit=fit<<-..y..)
        ylim(0,max ) +
    # ggtitle("") +
...
```{r}
 ...
 Two_Balance <- ggplot(mRES, aes(x = Replicate_Number, y = log2(F1_Score), colour = Method)) +
  # geom_point()+
  scale_color_viridis(discrete = T)+
  # geom_line() +
    # geom_point() +
...
```{r repres, echo = F, fig.cap= "Simulation 3 results. Replications and variance type feasibility ranges. X axes; replicate number, y axes log2 transformed rank product. Each facet is created with set parameters for combined effect size ($CE = \\|\\mathbf{T}_{\\alpha + \\alpha\\beta}\\mathbf{P}_{\\alpha + \\alpha\\beta}^T\\|_F$) and random gaussian noise size ($E = \\|\\mathbf{E}\\|_F$). Dots are median performance across 20 random structures."}


# One_Balance# + AUCs #+ plot_layout(guides = "collect")
...
# ps1 <- grid.arrange(ps, left = "Rank Product", bottom = "Replicate Number                     ")
...
<!-- In Figure \@ref(fig:repres)  -->

It was expected that all geometric mean ranks converge towards a minimum with increasing numbers of replicates, see \@ref(fig:repres). Perfect detection of 12 spikes in top 12 candidates is achieved with a log2 geometric mean rank of 2.402955, MASCARA this was able to achieve this however the other methods were perturbed by the presence of the 30 non spike differential genes.With more replicates MASCARA always outperforms competitors. With higher combined effect size (CE) MASCARA outperforms the other methods but its performance is affected by random noise (E) size. 

In simulation 2 we fixed the number of differential genes to 30, however this is less than 2% of the total number of genes in each dataset. In the real data example more than 10% of of the total number of genes are differentially expressed. As shown in simulation 1 (Figure \@ref(fig:tc)) higher numbers of DE genes results in higher geometric mean rank of the spikes for correlations, ASCA and PLS.  MASCARA is designed specifically to mitigate the presence of large numbers of differential genes. Here we show that this is achieved.
...
<!-- As well as, detection of 5 other genes known to be involved in the pathway (expression profiles shown in heatmap of Figure \@ref(fig:Dataset1-Overview)). -->
...
# ref$Baits[-which(ref$Baits %in% c(baits,baits2))] <- "non-bait"
# ref$Baits[which(ref$Baits %in% baits)] <- "bait"
# ref$Baits[which(ref$Baits %in% baits2)] <- "test_bait"
...
#, fig.cap="Top: distribution and mean sd relationship. Bottom: scores and loadings of the combined effect; growth condition + interaction"
...
cor_val <- function(x){
  ...
  # res <- (sum(cor_res))   /length(y)^2

  # res <- mean(cor_res[upper.tri(cor_res)])

  return(cor_res)

}

# plot(ASCA_cands2[1:1253,2], ASCA_cands2[1:1253,3])


test <- cor_val(res_ASCAplus_nb$residuals[,pathway])  #res_ASCAplus_nb$residuals
# hist(test[upper.tri(test)])
...
# sPLS_cands$Feature <- rownames(sPLS_cands)
# sPLS_cands <- merge(sPLS_cands, ref, by = "Feature")

# ggplotly(loadingplot(sPLS_cands, meta, baits = Fref2$Feature[which(Fref2$Baits %in% c("Pathway","PSI"))], 
...
#replace this to  get cands from original bait axis plus optimised maxima
...
# unique(REF2$query_name[which(REF2$domain_name %in% rownames(sPLS_cands)[1:9])])

# REF2[which(REF2$domain_name %in% rownames(sPLS_cands)[1:9]),c(1,3,4,19)]
...
for(i in 1:9){
  ...
  # qqplots_pca[[i]] <- qqplot(cbind.data.frame("Bait" = X[,which(colnames(X) %in% rownames(residual_cands)[i])],
  #                       "Candidate" = X[,which(colnames(X) %in% baits[1])]),meta, 
  ...
  qqplots_resid_cor[[i]] <- qqplot(cbind.data.frame("Candidate" = X[,which(colnames(X) %in% rownames(residual_coexp)[i])],
    ...
    # qqplots_spls_SR[[i]] <- qqplot(cbind.data.frame("Candidate" = X[,which(colnames(X) %in% rownames(SR)[i])],
    #                     "Bait" = X[,which(colnames(X) %in% baits[1])]),meta, 
    #    aes(x = Bait, y = Candidate, colour = growth_condition, shape = time))
    # 
...
<!-- Future research on this topic would involve further development of the data simulator.  -->
