...
Baits <- names(V(g))
Baits[-which(Baits %in% temp)] <- "Other"
Baits[which(Baits %in% temp)] <- "baits"
...
realplot <- ggraph(tidynet, layout = coords) + 
  ...
  geom_node_point(
    aes(size = VIP, color = type, shape = Baits), show.legend=FALSE         #CHANGE THIS
  ) +
  geom_node_point(
    aes(color = type)    #, shape = Baits
  ) +
...
adj[type == "URP", type == "URP"]  <- 1
adj[type %in% c("POI","RP"), type %in% c("POI","RP")] <- 1

adj[type %in% c("POI","RP"), type == "URP"] <- -1
adj[type == "URP", type %in% c("POI","RP")] <- -1
```
...
badplot <- ggraph(tidynet, layout = coords) + 
  ...
  scale_edge_color_manual(values = edge_colours, guide = "none") +
  geom_node_point(
    aes(size = VIP, color = type, shape = Baits), show.legend=FALSE
  ) +
  geom_node_point(
    aes(color = type)
  ) +
