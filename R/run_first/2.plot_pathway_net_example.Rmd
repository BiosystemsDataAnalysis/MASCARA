...
Net[which(Net %in% baits)] <- "POI"
Net[which(Net %in% spikes)] <- "RP"

Net[-which(Net %in% c("POI","RP"))] <- "URP"


temp <- paste0("X_",c(1991:1992,1989,1985))   #501:510

Gene <- names(V(g))
Gene[-which(Gene %in% temp)] <- "Other"
Gene[which(Gene %in% temp)] <- "Baits"
...
realplot <- ggraph(tidynet, layout = coords) + 
  ...
  geom_node_point(
    aes(size = VIP, color = Net, shape = Gene), show.legend=FALSE        #CHANGE THIS
  ) +
  geom_node_point(
    aes(color = Net, shape = Gene)    #, shape = Baits
  ) +
  scale_color_manual(values = colours) + 
    scale_shape_manual(values = c(17,16)) +   #18,20
...
adj[Net == "URP", Net == "URP"]  <- 1
adj[Net %in% c("POI","RP"), Net %in% c("POI","RP")] <- 1



adj[Net %in% c("POI","RP"), Net == "URP"] <- -1
adj[Net == "URP", Net %in% c("POI","RP")] <- -1
```
...
#define correctly ordered type Net
Net <- V(g)$name
...
badplot <- ggraph(tidynet, layout = coords) + 
  ...
  scale_edge_color_manual(values = edge_colours, guide = "none") +
  geom_node_point(
    aes(size = VIP, color = Net, shape = Gene), show.legend=FALSE
  ) +
  geom_node_point(
    aes(color = Net, shape = Gene)
  ) +
  scale_color_manual(values = colours) + 
      scale_shape_manual(values = c(17,16)) +   #18,20
...
# png("../networktypes.png", height = 3600, width = 4800, res = 700)
