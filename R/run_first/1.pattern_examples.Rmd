...
#list(X,P,Pb,a_ab,PLOT, X_a_ab,time, Struc_E )

###plot error in sim_data[[5]] 
...
# sim_data[[1]] <- sim_data[[1]][,1850:2000]
...
# data <- (0.1 * interaction) + (1.5*time) + (0.5*ambient) + (0*noise)
# 
input <- data
# 
# input <- cbind.data.frame(input,input,input)
...
# input <- input + rnorm(nrow(input) * ncol(input),mean = 0, sd = 1)
#replace with simulation input
...
```{r}
MASCARA4_test_1 <- function(resids,ref, baits, spikes, ncands){
  ...
  # sPLS_bait_cands <- as.data.frame(UTP[order(UTP[,1], decreasing = TRUE),])  #_9 and below
...
MASCARA4_test_2 <- function(resids,ref, baits, spikes, ncands){
  ...
  sPLS_cands <- cbind.data.frame(abs(spls_res$loadings$X) %*% spls_res$prop_expl_var$Y, spls_res$loadings$X) #  * spls_res$prop_expl_var$X
  sPLS_cands <- sPLS_cands[order(sPLS_cands[,1], decreasing = TRUE),]
...
#PLS1 model with svd(baits)$T as response
...
#PLS1 model with svd(baits)$T as response and target projected deflation

# MASCARA_4 <- MASCARA4_test_4(ar[[3]], baits, spikes = spikes)
...
# #PLS2 model with target projected deflation
# 
# 
# 
...
# yloadings[3,2] <- -0.5*yloadings[3,2]   #pure data manipulation for demo purposes
...
b <- ggplot(loadings_r_baits, aes(x = Comp_1, y = Comp_2, colour = ID, shape = ID)) +
  ...
  # geom_point(data = yloadings, shape = 4, aes(x = Comp_1, y = Comp_2, size = 3 )) + 
  theme_bw()
...
#multiple PLS1 models with svd(baits) deflated


```

```{r}
#remake simpls to deflate with target projected loadings 

#first recreate simpls such that the output is the same as simpls.fit()
